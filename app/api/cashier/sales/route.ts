...
export async function GET(req: Request) {
  try {
    ...
    if (search) {
      ...
      const isPaymentMethod = ['CASH', 'MOMO', 'CARD'].includes(searchUpper)
      ...
      where.OR = [
        ...
        },
        {
          payments: {
            some: {
              OR: [
                ...(isPaymentMethod ? [{ method: { equals: searchUpper as any } }] : []),
                {
                  reference: {
                    contains: search,
                    mode: 'insensitive'
    ...
    const mappedOrders = orders.map(order => ({
      ...
      payment_method: order.payments[0]?.method?.toLowerCase() || 'other',
      payment_reference: order.payments[0]?.reference || null,
